...
global function GenerateTitanOSAlias_custom
...
void function OS_Running( entity player )
{
	...
	while ( true )
	{
		...
		else if ( ( player.GetPetTitan() != null ) )
		{
			...
			{
				/*
				EmitSoundOnEntity( player, "music_wilds_16d_embark" )
				OS_Print( "Playing music" )
				FadeOutSoundOnEntity( player, "music_wilds_16d_embark", 2 )
				*/
...
void function OS_BatteryRadar( entity player )
{
	...
	while ( true )
	{
		...
		float delay = GetConVarFloat( "OSR.Battery_Nag_Delay" )
		...
		foreach ( entity battery in batteries )
		{
			...
			if ( file.is_boarded )
			{
				...
				{
					string conversationName = "batteryNearDisembark"
					PlayOneLinerConversationOnEntWithPriority_custom( soundAlias, player, 275 )
					wait delay
				}
				else if ( dist3d < 2600 )
				{
					string conversationName = "batteryNearGeneric"
					PlayOneLinerConversationOnEntWithPriority_custom( soundAlias2, player, 275 )
					wait delay
				}
			}
			else
...
void function OS_DoomBeep( entity player )
{
	...
	//FadeOutSoundOnEntity( player, "boneyard_scr_bliskintro_foley_part1", 1.0 )
				...
				if( player.IsTitan() )
	 			{
					...
					alarmIsPlaying = true
					PlayOneLinerConversationOnEntWithPriority_custom( soundAlias, player, 1000)
					wait 0.6
		...
		if ( rider == null )
		{
			...
			if ( hostileOnBoard )
			{
				...
				if( !BoardedWithBattery )
				{
					...
					//OS_Print("the battery status of " + prevrider + "is " + CheckEntityBattery( prevrider ) )
	
					if( CheckEntityBattery( prevrider ) )
					{
						//OS_Print("debug riderz 2")
						int random = RandomInt( 2 )
						if ( random == 0 )
						{
							PlayOneLinerConversationOnEntWithPriority_custom( soundAlias_stolengnrc, player, 2001 )
						}
						else
						{
							PlayOneLinerConversationOnEntWithPriority_custom( soundAlias_stolen, player, 2001 )
						}
					}
					else
					{
						...
						if ( IsAlive( prevrider ) )
						{
							...
							PlayOneLinerConversationOnEntWithPriority_custom( soundAlias_left, player, 2001 )
						}
						else
						{
							...
							if ( random == 0 )
							{
								PlayOneLinerConversationOnEntWithPriority_custom( soundAlias_dead, player, 2001 )
							}
							else
							{
								PlayOneLinerConversationOnEntWithPriority_custom( soundAlias_deadgnrc, player, 2001 )
							}
...
void function PlayOneLinerConversationOnEntWithPriority_custom_delayed( string soundAlias, entity ent, int priority, float delay )
{
	wait delay

	PlayOneLinerConversationOnEntWithPriority_custom( soundAlias, ent, priority )
}
